<?php
    ...
    function get_login($name,$pass){
        global $checkLogin;
        $check=false;      
        for($i=0;$i<count(get_all_Use());$i++){
            if(get_all_Use()[$i][1]==$name && get_all_Use()[$i][2]==$pass){
                ?>
                <script>
        ...
        for($i=0;$i<count(get_all_Admin());$i++){
            if(get_all_Admin()[$i][1]==$name && get_all_Admin()[$i][2]==$pass){
                ?>
                <script>
    ...
    function  addUser($name,$adr,$user,$pass){
        global $db;
        connect_db();
        $sql="INSERT into User values(null,'$user','$pass','$name','$adr')";
        $db->query($sql);
    }
    function getNumCart(){
        global $db;
    ...
    function getCustomer(){
        global $db;
        connect_db();
        $sql="SELECT * FROM Oder GROUP BY idUser";
        $result = $db->query($sql)->fetch_all();
        return $result;
    }
    ...
    function insertOderBought(){
        ...
        $count=0;
        for ($i = 0; $i < count(get_all_Cart()); $i++) {
            $sql="INSERT into oderBought values (null,".get_all_Cart()[$i][1].",".get_all_Cart()[$i][2].",".get_all_Cart()[$i][3].",'2017-06-15')";
            $count=$count+1;
            $db->query($sql);
        }   
